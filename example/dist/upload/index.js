...
VantComponent({
    ...
    methods: {
        ...
        chooseImage() {
            let { count = 9, sizeType = ["original", "compressed"], sourceType = ["album", "camera"] } = this.data.imageOption;
            wx.chooseImage({
                count,
                sizeType,
                sourceType,
                success: res => {
                    ...
                    while (newFiles.length < count && file) {
                        // 处理file
                        file = this.processFile(Object.assign({}, file), TYPE_IMAGE);
                        if (!file.ignore) {
                            newFiles.push(file);
                            this.set({
        ...
        chooseVideo() {
            let { sourceType = ["album", "camera"], compressed = true, maxDuration = 60, camera = "back" } = this.data.videoOption;
            wx.chooseVideo({
                sourceType,
                compressed,
                maxDuration,
                ...
                success: res => {
                    // 选择完文件后触发，一般可用作文件过滤
                    this.$emit(EVENT_ADDED, res);
                    let file = this.processFile(Object.assign({}, res), TYPE_VIDEO);
                    if (!file.ignore) {
                        this.set({
                            files: this.data.files.concat(file)
                        }).then(() => {
